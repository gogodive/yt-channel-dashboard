...
- 📌 **노션 상태 표기**: 실행 끝에 노션 분석 페이지 최상단 콜아웃을
...
2. 대상 노션 페이지(config.yaml 의 `hub_page_id`) → 우측 상단 ··· → **연결** → 만든 integration 추가
...
- **노션 콜아웃이 안 바뀜** — `NOTION_TOKEN` 시크릿 등록 여부, 대상 페이지에 integration 을
  '연결'했는지, 콜아웃에 "대시보드 자동 갱신" 문구가 남아 있는지 확인
